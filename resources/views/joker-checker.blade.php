...
@section('title', 'Joker Combination Checker')

@section('content')
    <header class="text-center mb-12">
        <img src="{{ asset('img/joker.jpg') }}" alt="Joker Logo" class="mx-auto" style="max-height: 150px;">
        <h1 class="text-3xl font-bold mt-4">Joker Combination Checker</h1>
    ...
    <div x-data="checkerApp()" class="space-y-8">
        ...
        <section class="card-glass rounded-3xl p-8">
            <form id="checker-form" action="{{ route('joker.checker') }}" method="GET" @submit.prevent="validateForm($event)">
                @if($results)
                ...
                @else
                    <div class="grid md:grid-cols-2 gap-8">
                        ...
                        <!-- Joker Selection -->
                        <div>
                            <h3 class="text-xl font-semibold mb-4 text-yellow-400">Joker (Select 1)</h3>
                            <div class="grid grid-cols-10 gap-2">
                ...
                @if(count($results['match_history']) > 0)
                    <div class="card-glass rounded-3xl overflow-hidden">
                        <div class="p-6 border-b border-slate-700">
                            ...
                            <p class="text-sm text-slate-400">Showing draws with significant matches (e.g. 5, 4+1, 3+1 etc.)</p>
                        </div>
                                ...
                                <tbody class="divide-y divide-slate-700">
                                    @foreach($results['match_history'] as $match)
                                        <tr class="hover:bg-slate-700/30 transition-colors">
                                            ...
                                            <td class="px-6 py-4">
                                                <span class="px-3 py-1 rounded-full text-xs font-bold {{ $match['tier'] == '5+1' ? 'bg-yellow-500/20 text-yellow-500 border border-yellow-500' : 'bg-blue-500/20 text-blue-400 border border-blue-500' }}">
                                                    {{ $match['tier'] }}
                                                </span>
                                            </td>
                                        </tr>
        ...
        function checkerApp() {
            return {
                ...
                toggleJoker(n) {
                    ...
                    } else if (this.selectedJokers.length < 1) {
                ...
                quickPick() {
                    ...
                    while (this.selectedJokers.length < 1) {
                ...
                validateForm(e) {
                    if (this.selectedNumbers.length !== 5 || this.selectedJokers.length !== 1) {
                        this.error = 'Please select exactly 5 main numbers and 1 Joker.';
                    } else {
